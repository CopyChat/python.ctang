...
#Longitude=[str(a) for a in range(1,len(GCMvar3D[0,0,:])+1)]
#print Longitude
...
#=================================================== to DataFrame
    ...
    #       to datetime objects
    GCMdf=pd.DataFrame(GCMvar3D[:,j,:])
    #GCMdf=pd.DataFrame(np.zeros(1440*lenLon).reshape(1440,lenLon))
...
#=================================================== to DataFrame
    ...
    # NOTE: cannot convert 360_day np.arrary objects read from netcdf
    #       to datetime objects
    RELYdf=pd.DataFrame(RELYvar3D[:,j,:],dtype='float32')
    ...
    #print RELYdf.dtypes
    #print RELYdf.iloc[0,:]
    #print RELYdf.iloc[0,0:lenLon].shape #196
    ##quit()
    #print np.array(GCMdf[(GCMdf['year'] == RELYdf['year'][0]) 
        #& (GCMdf['month'] == RELYdf['month'][0])])
    #print np.array(GCMdf[(GCMdf['year'] == RELYdf['year'][0]) 
            #& (GCMdf['month'] == RELYdf['month'][0])])[:,0:lenLon].shape # 119
    ##quit()
    ##print [t for t in np.array(GCMdf[(GCMdf['year'] == RELYdf['year'][0]) 
        ##& (GCMdf['month'] == RELYdf['month'][0])][:])]
    #print np.array([np.subtract(t,RELYdf.iloc[0,0:lenLon]) 
        #for t in np.array(GCMdf[(GCMdf['year'] == RELYdf['year'][0]) 
            #& (GCMdf['month'] == RELYdf['month'][0])])[:,0:lenLon]])
    #print np.array([np.subtract(t,RELYdf.iloc[0,0:lenLon]) 
        #for t in np.array(GCMdf[(GCMdf['year'] == RELYdf['year'][0]) 
            #& (GCMdf['month'] == RELYdf['month'][0])])[:,0:lenLon]]).shape
    ...
    for t in [1,20]:
        #print RELYdf.index
        MonthlyMeanBias=np.array([np.subtract(x,RELYdf.iloc[t,0:lenLon]) 
            for x in np.array(GCMdf[
            (GCMdf['year'] == RELYdf['year'][t]) &
            (GCMdf['month'] == RELYdf['month'][t]) &
            (GCMdf['hour'] == RELYdf['hour'][t])
            ])[:,0:lenLon]])
        # NOTE: all well untill this line:

        print "GCMvar3D2 wrong1:"
        print [x for x in GCMvar3D[0:30,j,:]] # right
        print "GCMdf:wrong"
...
#=================================================== 
        ...
        #print np.array(RELYdf.iloc[t,0:lenLon]) 
        ...
        MeanBias[K:K+L,j,:]=MonthlyMeanBias
        #print " MeanBias           ======="
        #print MeanBias[K:K+L,j,:]
        print " time = "+str(RELYtime[t])+" t= "+str(t)+", L= "+str(L)+", MeanBias len= "+str(len(MeanBias[K:K+L,0,0]))+" K= " +str(K)+", end= "+str(K+L) + " latitude index = "+str(j)
        K=K+L
    ...
    #GCMdf=pd.DataFrame(np.zeros(1440*lenLon).reshape(1440,lenLon))
        ...
        lonindex1=np.random.randint(0,lenLon/2)
        #dateindex2=np.random.randint(L/2,L)
        #lonindex2=np.random.randint(L/2,lenLon)
        ...
        #print '============='
        print '============= GCM values in checking'
        print GCMvalue[:,lonindex1:lonindex1+20]
        print '============= MonthlyMeanBias in checking'
        print MonthlyMeanBias[:,lonindex1:lonindex1+20]
        #print '============='
        print '============='
        print "GCM value - MeanBiasValue     =  "+str(GCMvalue[:,lonindex1:lonindex1+20]-MonthlyMeanBias[:,lonindex1:lonindex1+20])
        print "Defaule         RELYvalue     =  "+str(RELYdf.iloc[t,lonindex1:lonindex1+20])
            #for x in np.array(MonthlyMeanBias)[:,:]])[np.random.randint(0,L),np.random.randint(0,lenLon)]
#=================================================== print results
#print "========================= GCM data:=========================="
#print GCMvar3D
